...
keynote.nav = {
		init:function(keyParam,paramNav,index){
			(typeof(keyParam)=='undefined')&&(keyParam = keynote.param());
			...
			if(!paramNav.private){
				...
				paramNav.currentLi = keyParam.currentLi;
				paramNav.preLi = keyParam.preLi;
					...
					paramNav.li[i].addEventListener(StartEvent,function(){
						...
						if(index!=null){
							for (var i = 0; i < navList.length; i++){
								...
								navList[i].navLiHover(keyParam,navList[i]);
								keyParam.callBack(keyParam);
							}
						}else{
							paramNav.currentLi = seq-1;
							paramNav.navLiHover(keyParam,paramNav);
							keyParam.callBack(keyParam);
						}
						
					})
				}
				this.navLiHover(keyParam,paramNav);
			}
			return paramNav;
		},
		keyLiHover : function(keyParam,paramNav){
		...
		navLiHover :function(keyParam,paramNav){
			this.keyLiHover(keyParam,paramNav);
			keyParam.ul.style.webkitTransition = 'none';
			keyParam.margin = paramNav.currentLi*(-keyParam.liwidth);
			keyParam.ul.style.marginLeft = keyParam.margin+'px';
			keyParam.preLi = keyParam.currentLi;
			keyParam.currentLi = paramNav.currentLi;
			setTimeout(function(){
				keyParam.ul.style.webkitTransition = '';
			},20)
